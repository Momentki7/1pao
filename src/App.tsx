...
const Header = ({ title, showSearch = true, onAddClick }: { title: string, showSearch?: boolean, onAddClick?: () => void }) => (
  <header className="sticky top-0 z-20 w-full px-4 pt-6 pb-1.5 flex items-center justify-between bg-misty-pink/80 ios-blur border-b border-misty-pink-dark/30">
    ...
    <div className="relative overflow-hidden w-full bg-slate-100 border-b border-misty-pink/30">
      ...
      <div className="absolute inset-y-0 right-0 flex">
        <button 
          ...
          className="bg-slate-500 text-white w-16 flex flex-col items-center justify-center text-xs font-medium active:bg-slate-600 transition-colors"
        >
        ...
        <div className="relative">
          {chat.avatar ? (
            <img 
              src={chat.avatar} 
              alt={chat.name} 
              className="w-14 h-14 object-cover shadow-sm border border-slate-100 bg-slate-100"
              style={{ borderRadius: `${chat.avatarRadius === undefined ? 24 : chat.avatarRadius / 2}%` }}
              referrerPolicy="no-referrer"
            />
          ) : (
            <div className="w-14 h-14 shadow-sm border border-slate-200 bg-slate-100 flex items-center justify-center text-slate-400" style={{ borderRadius: `${chat.avatarRadius === undefined ? 24 : chat.avatarRadius / 2}%` }}>
              <User size={28} />
            </div>
          )}
      ...
      {sortedChats.length === 0 ? (
        ...
        sortedChats.map((chat) => (
          <SwipeableChatItem 
            ...
            onRequestReply={() => onRequestReply(chat)}
          />
        ))
      )}
    </motion.div>
...
const ProfileView = ({ onOpenSettings, onOpenWorldBook }: { onOpenSettings: () => void, onOpenWorldBook: () => void }) => (
  <motion.div 
    initial={{ opacity: 0, y: 10 }}
    animate={{ opacity: 1, y: 0 }}
    exit={{ opacity: 0, y: -10 }}
  ...
  >
    <div className="bg-white/80 px-4 py-8 flex items-center gap-4 border-b border-misty-pink/30">
      <div className="w-20 h-20 rounded-2xl shadow-md border border-slate-200 bg-slate-100 flex items-center justify-center text-slate-400">
        <User size={40} />
      </div>
    ...
    <div className="bg-white/60 border-y border-misty-pink/30">
      ...
      >
        <div className="flex items-center gap-3">
          <Book size={22} className="text-misty-pink-dark" />
          <span className="text-[17px]">世界书</span>
        </div>
        <ChevronRight size={18} className="text-ios-gray" />
      </div>
    </div>
...
const WorldBookView = ({ onBack }: { onBack: () => void }) => {
  const [worldBooks, setWorldBooks] = useLocalStorage<WorldBook[]>('sjzn_worldbooks', []);
  const [showAddModal, setShowAddModal] = useState(false);
  const [editingBook, setEditingBook] = useState<WorldBook | null>(null);
  const groupedBooks = worldBooks.reduce((acc, book) => {
  ...
  const [expandedCategories, setExpandedCategories] = useState<Record<string, boolean>>(() => {
    ...
    Object.keys(groupedBooks).forEach(cat => {
      initial[cat] = groupedBooks[cat].length === 1;
    });
    return initial;
  });
  ...
  const [draftPosition, setDraftPosition] = useState<'before' | 'middle' | 'after'>('before');
  ...
  const handleSave = () => {
    ...
    if (editingBook) {
      setWorldBooks(worldBooks.map(b => b.id === editingBook.id ? {
        ...b,
  ...
  const handleDelete = (id: string) => {
    if (confirm('确定要删除这条世界书吗？')) {
      setWorldBooks(worldBooks.filter(b => b.id !== id));
    }
  };

  const openAddModal = (book?: WorldBook) => {
    if (book) {
      setEditingBook(book);
      setDraftCategory(book.category);
      setDraftName(book.name);
      setDraftContent(book.content);
      setDraftIsGlobal(book.isGlobal);
      setDraftPosition(book.position);
    } else {
      setEditingBook(null);
      setDraftCategory('');
      setDraftName('');
      setDraftContent('');
      setDraftIsGlobal(false);
      setDraftPosition('before');
    }
  ...
  return (
    ...
    >
      <header className="sticky top-0 z-20 w-full px-4 pt-6 pb-1.5 flex items-center justify-between bg-misty-pink/80 ios-blur border-b border-misty-pink-dark/30">
        <button onClick={onBack} className="flex items-center text-slate-800 -ml-2 active:opacity-50 transition-opacity">
          <ChevronLeft size={28} />
          <span className="text-[17px]">返回</span>
        </button>
        <h1 className="text-[17px] font-semibold absolute left-1/2 -translate-x-1/2 text-slate-800">世界书</h1>
        <button onClick={() => openAddModal()} className="text-slate-600 active:opacity-50 transition-opacity">
          <Plus size={24} />
        </button>
      </header>
            ...
            {Object.entries(groupedBooks).map(([category, books]) => (
              <div key={category} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <button 
                  ...
                  className="w-full px-4 py-3 flex items-center justify-between bg-slate-50/50 active:bg-slate-100 transition-colors"
                >
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-slate-800">{category}</span>
                    <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{books.length}</span>
                  </div>
                  <ChevronRight size={18} className={`text-slate-400 transition-transform ${expandedCategories[category] ? 'rotate-90' : ''}`} />
                </button>
                  ...
                  {expandedCategories[category] && (
                    ...
                    >
                      <div className="p-2 flex flex-col gap-2">
                        {books.map(book => (
                          <div key={book.id} className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                            <div className="flex justify-between items-start mb-2">
                              <div>
                                <h3 className="font-semibold text-slate-800 text-[15px]">{book.name}</h3>
                              </div>
                              <div className="flex items-center gap-2">
                                {book.isGlobal && <span className="text-[10px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded">全局</span>}
                                <button onClick={() => openAddModal(book)} className="text-slate-400 hover:text-slate-600"><Settings size={16} /></button>
                                <button onClick={() => handleDelete(book.id)} className="text-red-400 hover:text-red-600"><Trash2 size={16} /></button>
                              </div>
                            </div>
                            <p className="text-sm text-slate-500 line-clamp-3">{book.content}</p>
                          </div>
                        ))}
                      </div>
                    </motion.div>
        ...
        {showAddModal && (
          ...
          >
            <motion.div 
              ...
              className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-xl flex flex-col max-h-[80vh]"
            >
              <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-[#F3F3F3]">
                <h3 className="text-lg font-semibold text-slate-800">{editingBook ? '编辑世界书' : '添加世界书'}</h3>
              ...
              <div className="p-4 overflow-y-auto flex-1 flex flex-col gap-4">
                <div>
                  <label className="text-sm font-medium text-slate-700 block mb-1">分类</label>
                  <input 
                    type="text" 
                    value={draftCategory}
                    onChange={e => setDraftCategory(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-misty-pink-dark"
                    placeholder="选填"
                  />
                </div>
                <div>
                  <label className="text-sm font-medium text-slate-700 block mb-1">名称</label>
                  <input 
                    type="text" 
                    value={draftName}
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-misty-pink-dark min-h-[100px]"
                    placeholder="必填，描述世界规则或设定"
                  />
                </div>
                
                <div className="flex items-center justify-between mt-2">
                  <label className="text-sm font-medium text-slate-700">全局生效</label>
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-misty-pink-dark"></div>
                  </label>
                </div>

                <div>
                  <label className="text-sm font-medium text-slate-700 block mb-2">注入位置</label>
                  <div className="flex gap-2">
                    <button 
                      onClick={() => setDraftPosition('before')}
                      className={`flex-1 py-1.5 rounded-lg text-sm transition-colors border ${draftPosition === 'before' ? 'bg-misty-pink-dark text-white border-misty-pink-dark' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                      前
                    </button>
                    <button 
                      onClick={() => setDraftPosition('middle')}
                      className={`flex-1 py-1.5 rounded-lg text-sm transition-colors border ${draftPosition === 'middle' ? 'bg-misty-pink-dark text-white border-misty-pink-dark' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                      中
                    </button>
                    <button 
                      onClick={() => setDraftPosition('after')}
                      className={`flex-1 py-1.5 rounded-lg text-sm transition-colors border ${draftPosition === 'after' ? 'bg-misty-pink-dark text-white border-misty-pink-dark' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                      后
                    </button>
                  </div>
                  <p className="text-[10px] text-slate-400 mt-1">前：最先读取；中：人设之后；后：最后读取</p>
                </div>
              </div>
              ...
              <div className="p-4 border-t border-slate-100 bg-white">
                <button 
                  ...
                  className="w-full py-3 bg-misty-pink text-slate-800 rounded-xl font-medium disabled:opacity-50 active:opacity-80 transition-opacity shadow-sm"
                >
                  保存
                </button>
              </div>
...
const SettingsView = ({ onBack }: { onBack: () => void }) => {
  ...
  return (
    <motion.div 
      ...
      className="absolute inset-0 bg-slate-100 z-50 flex flex-col"
    >
      <header className="sticky top-0 z-20 w-full px-4 pt-6 pb-1.5 flex items-center bg-misty-pink/80 ios-blur border-b border-misty-pink-dark/30">
...
const ChatInfo = ({ chat, messages, setMessages, onBack, onJumpToMessage, onUpdateChat }: { chat: ChatItem, messages: Message[], setMessages: (msgs: Message[]) => void, onBack: () => void, onJumpToMessage: (id: string) => void, onUpdateChat: (id: string, updates: Partial<ChatItem>) => void }) => {
  const [draftChat, setDraftChat] = useState<ChatItem>(chat);
  const [activeTab, setActiveTab] = useState('设置');
  const [showSearchHistory, setShowSearchHistory] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [showClearConfirm, setShowClearConfirm] = useState(false);

  // Load sticker groups to bind
  const [stickerGroups] = useLocalStorage<{id: string, name: string}[]>('sjzn_sticker_groups', [{ id: 'default', name: '未分类' }]);
  const [worldBooks] = useLocalStorage<WorldBook[]>('sjzn_worldbooks', []);
  ...
  const handleSave = () => {
    onUpdateChat(chat.id, draftChat);
    setShowToast('保存成功');
    setTimeout(() => {
      setShowToast('');
      onBack();
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>, field: 'avatar' | 'userAvatar') => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        setDraftChat({ ...draftChat, [field]: event.target?.result as string });
      };
      reader.readAsDataURL(file);
    }
  };

  const toggleStickerGroup = (groupId: string) => {
    const current = draftChat.boundStickerGroups || [];
    if (current.includes(groupId)) {
      setDraftChat({ ...draftChat, boundStickerGroups: current.filter(id => id !== groupId) });
    } else {
      setDraftChat({ ...draftChat, boundStickerGroups: [...current, groupId] });
    }
  };

  const toggleWorldBook = (bookId: string) => {
    const current = draftChat.boundWorldBooks || [];
    if (current.includes(bookId)) {
      setDraftChat({ ...draftChat, boundWorldBooks: current.filter(id => id !== bookId) });
    } else {
      setDraftChat({ ...draftChat, boundWorldBooks: [...current, bookId] });
    }
  };
  ...
  return (
    ...
    >
      {/* Header */}
      <header className="pt-6 pb-1.5 px-4 bg-[#F3F3F3] flex items-center justify-between">
      ...
      <div className="flex-1 overflow-y-auto pb-8 px-4">
        ...
        <div className="flex flex-col items-center justify-center pt-6 pb-8">
          <label className="relative cursor-pointer mb-3">
            <input type="file" accept="image/*" className="hidden" onChange={(e) => handleAvatarChange(e, 'avatar')} />
            {draftChat.avatar ? (
              <img src={draftChat.avatar} className="w-20 h-20 object-cover border border-slate-100 shadow-sm" style={{ borderRadius: `${draftChat.avatarRadius === undefined ? 24 : draftChat.avatarRadius / 2}%` }} />
            ) : (
              <div className="w-20 h-20 bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 shadow-sm" style={{ borderRadius: `${draftChat.avatarRadius === undefined ? 24 : draftChat.avatarRadius / 2}%` }}>
                <User size={40} />
              </div>
            )}
          </label>
          <input
            type="text"
            value={draftChat.remark || draftChat.name}
            onChange={e => setDraftChat({...draftChat, remark: e.target.value})}
            onKeyDown={e => {
              if (e.key === 'Enter') {
            ...
            className="text-[18px] font-medium text-slate-800 bg-transparent text-center outline-none w-full max-w-[200px]"
            placeholder="设置备注"
          />
        </div>

        {/* Search History */}
        <div className="bg-white rounded-2xl mb-4 overflow-hidden mt-2">
          <button 
          ...
          <div className="flex border-b border-slate-100">
            {['设置', '美化', '记忆', '其他'].map(tab => (
              <button 
                ...
                className={`flex-1 py-3 text-sm font-medium transition-colors ${activeTab === tab ? 'text-slate-800 border-b-2 border-slate-800 bg-slate-50/50' : 'text-slate-400 hover:bg-slate-50'}`}
              >
                {tab}
              </button>
            ))}
          ...
          <div className="p-4">
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[15px] text-slate-800 font-medium">对方头像</span>
                    <label className="relative cursor-pointer">
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => handleAvatarChange(e, 'avatar')} />
                      {draftChat.avatar ? (
                        <img src={draftChat.avatar} className="w-12 h-12 object-cover border border-slate-100 shadow-sm" style={{ borderRadius: `${draftChat.avatarRadius === undefined ? 24 : draftChat.avatarRadius / 2}%` }} />
                      ) : (
                        <div className="w-12 h-12 bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 shadow-sm" style={{ borderRadius: `${draftChat.avatarRadius === undefined ? 24 : draftChat.avatarRadius / 2}%` }}>
                          <User size={24} />
                        </div>
                      )}
                  ...
                  <div>
                    <span className="text-[13px] text-slate-500 mb-1.5 block">对方人设</span>
                    <textarea 
                      value={draftChat.persona || ''}
                      onChange={e => setDraftChat({...draftChat, persona: e.target.value})}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-[14px] outline-none focus:border-slate-400 min-h-[60px] resize-none"
                      placeholder="输入对方的人设..."
                    />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[15px] text-slate-800 font-medium">我的头像</span>
                    <label className="relative cursor-pointer">
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => handleAvatarChange(e, 'userAvatar')} />
                      {draftChat.userAvatar ? (
                        <img src={draftChat.userAvatar} className="w-12 h-12 object-cover border border-slate-100 shadow-sm" style={{ borderRadius: `${draftChat.avatarRadius === undefined ? 24 : draftChat.avatarRadius / 2}%` }} />
                      ) : (
                        <div className="w-12 h-12 bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 shadow-sm" style={{ borderRadius: `${draftChat.avatarRadius === undefined ? 24 : draftChat.avatarRadius / 2}%` }}>
                          <User size={24} />
                        </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[15px] text-slate-800 font-medium">头像圆角</span>
                    <span className="text-[13px] text-slate-400">{draftChat.avatarRadius === undefined ? 48 : draftChat.avatarRadius}%</span>
                  </div>
                  <input 
                    type="range" 
                    min="0" 
                    max="100" 
                    value={draftChat.avatarRadius === undefined ? 48 : draftChat.avatarRadius}
                    onChange={e => setDraftChat({...draftChat, avatarRadius: parseInt(e.target.value)})}
                ...
                {/* Sticker Binding */}
                <div>
                  <span className="text-[15px] text-slate-800 font-medium mb-3 block">表情包绑定</span>
                  <div className="flex flex-wrap gap-2">
                    {stickerGroups.map(g => (
                      <button 
                        key={g.id}
                        onClick={() => toggleStickerGroup(g.id)}
                        className={`px-3 py-1.5 rounded-full text-[13px] transition-colors border ${draftChat.boundStickerGroups?.includes(g.id) ? 'bg-misty-pink-dark text-white border-misty-pink-dark' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                      >
                        {g.name}
                      </button>
                    ))}
                ...
                {/* World Book Binding */}
                <div>
                  <span className="text-[15px] text-slate-800 font-medium mb-3 block">世界书绑定</span>
                  <div className="flex flex-col gap-2">
                    {worldBooks.length === 0 ? (
                      <span className="text-[13px] text-slate-400">暂无世界书，请在“我的-世界书”中添加</span>
                    ) : (
                      worldBooks.map(book => (
                        <div key={book.id} className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-100">
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-[14px] text-slate-800 font-medium">{book.name}</span>
                              {book.isGlobal && <span className="text-[10px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded">全局</span>}
                            </div>
                            <span className="text-[12px] text-slate-500 line-clamp-1 mt-0.5">{book.content}</span>
                          </div>
                          <button 
                            onClick={() => toggleWorldBook(book.id)}
                            disabled={book.isGlobal}
                            className={`px-3 py-1.5 rounded-full text-[12px] transition-colors border ${book.isGlobal ? 'bg-slate-200 text-slate-400 border-slate-200' : draftChat.boundWorldBooks?.includes(book.id) ? 'bg-misty-pink-dark text-white border-misty-pink-dark' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                          >
                            {book.isGlobal ? '已全局绑定' : draftChat.boundWorldBooks?.includes(book.id) ? '已绑定' : '绑定'}
                          </button>
                        </div>
                      ))
                    )}
            ...
            {activeTab !== '设置' && (
              <div className="py-12 flex flex-col items-center justify-center text-slate-400">
                ...
                <span className="text-[14px]">{activeTab}功能开发中...</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl overflow-hidden mb-8">
          <button 
            ...
            className="w-full px-4 py-4 text-left text-[16px] text-red-500 active:bg-slate-50 transition-colors border-b border-slate-100/60"
          >
            清空聊天记录
          </button>
          <button 
            className="w-full px-4 py-4 text-left text-[16px] text-slate-800 active:bg-slate-50 transition-colors"
          >
            投诉
          </button>
        </div>
        ...
        <button 
          onClick={handleSave}
          className="w-full bg-misty-pink text-slate-800 font-medium text-[16px] py-3.5 rounded-2xl active:opacity-80 transition-opacity mb-8 shadow-sm"
        >
          保存设置
        </button>
      </div>
      ...
      <AnimatePresence>
        {showSearchHistory && (
          ...
          >
            ...
            <header className="pt-6 pb-1.5 px-4 bg-white flex items-center gap-3 border-b border-slate-100">
              ...
              <button onClick={() => { setShowSearchHistory(false); setSearchQuery(''); }} className="text-[15px] text-slate-800 font-medium whitespace-nowrap">
                取消
              </button>
            </header>
            ...
            <div className="flex-1 overflow-y-auto bg-white">
              {searchQuery ? (
                searchResults.length > 0 ? (
                  searchResults.map(msg => (
                      ...
                      onClick={() => {
                        ...
                        setTimeout(() => {
                          ...
                          if (el) {
                            ...
                            setTimeout(() => el.classList.remove('bg-slate-200'), 2000);
                          }
                        }, 300);
                      }}
                    ...
                    >
                      {msg.sender === 'user' ? (
                        chat.userAvatar ? (
                          <img src={chat.userAvatar} className="w-10 h-10 object-cover border border-slate-100 shadow-sm bg-slate-100 flex-shrink-0" style={{ borderRadius: `${chat.avatarRadius === undefined ? 24 : chat.avatarRadius / 2}%` }} />
                        ) : (
                          <div className="w-10 h-10 bg-slate-100 border border-slate-200 shadow-sm flex items-center justify-center text-slate-400 flex-shrink-0" style={{ borderRadius: `${chat.avatarRadius === undefined ? 24 : chat.avatarRadius / 2}%` }}>
                            <User size={20} />
                          </div>
                        )
                      ) : chat.avatar ? (
                        <img src={chat.avatar} className="w-10 h-10 object-cover border border-slate-100 shadow-sm bg-slate-100 flex-shrink-0" style={{ borderRadius: `${chat.avatarRadius === undefined ? 24 : chat.avatarRadius / 2}%` }} />
                      ) : (
                        <div className="w-10 h-10 bg-slate-100 border border-slate-200 shadow-sm flex items-center justify-center text-slate-400 flex-shrink-0" style={{ borderRadius: `${chat.avatarRadius === undefined ? 24 : chat.avatarRadius / 2}%` }}>
                          <User size={20} />
                        </div>
                ...
                <div className="p-8 text-center text-slate-400 text-[15px]">
                  目前共有 {messages.length} 条聊天记录
                </div>
              )}
            </div>
          </motion.div>
...
const ChatRoom = ({ chat, onBack, onUpdateChat, onBackgroundReply }: { chat: ChatItem, onBack: () => void, onUpdateChat: (id: string, updates: Partial<ChatItem>) => void, onBackgroundReply: (chatId: string, message: Message) => void }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('sjzn_bg_message', handleBgMessage);
    return () => window.removeEventListener('sjzn_bg_message', handleBgMessage);
  }, [chat.id]);
  ...
  useEffect(() => {
    // Update chat list with last message and time
    if (messages.length > 0) {
      const lastMessage = messages[messages.length - 1];
      let lastMsgText = lastMessage.type === 'text' ? lastMessage.content : '[动画表情]';
      if (lastMessage.type === 'image') lastMsgText = '[图片]';
      
      onUpdateChat(chat.id, {
        lastMsg: lastMsgText,
        time: new Date(lastMessage.timestamp).toISOString(),
  ...
  const handleRequestReply = async () => {
    // 3. 先默认使用全局
    const apiConfig = settings.globalApi;
    ...
    // Load world books
    const allWorldBooks = JSON.parse(localStorage.getItem('sjzn_worldbooks') || '[]') as WorldBook[];
    const boundWorldBooks = allWorldBooks.filter(b => b.isGlobal || chat.boundWorldBooks?.includes(b.id));
    
    const beforeBooks = boundWorldBooks.filter(b => b.position === 'before').map(b => b.content).join('\n\n');
    const middleBooks = boundWorldBooks.filter(b => b.position === 'middle').map(b => b.content).join('\n\n');
    const afterBooks = boundWorldBooks.filter(b => b.position === 'after').map(b => b.content).join('\n\n');
    ...
    const boundStickers = stickers.filter(s => chat.boundStickerGroups?.includes(s.groupId));
    const stickersPrompt = boundStickers.length > 0 
      ? `\n[可用表情包]\n你只能从以下表情包中选择发送（若不需要发表情包则不发）。如果发送表情包，表情包必须单独占一行，格式严格为：[表情包:表情包名字] （注意是英文冒号，且不要有空格）。\n可用表情包列表：\n${boundStickers.map(s => `- ${s.name}`).join('\n')}\n注意：绝对不能捏造列表外的表情包！按照表情包的名字和自己的心情契合度，在合适的时候发出表情包，和真人聊天类似。\n`
      : '';
    
    const prompt = `
${beforeBooks ? `[World Rules (Before)]\n${beforeBooks}\n\n` : ''}You ARE ${chat.trueName || chat.name || 'this character'}. This is not a roleplay, you are literally them.
...
Before replying, you MUST think through the 8 steps internally. 
You MUST wrap your ENTIRE thinking process STRICTLY inside <think> and </think> tags. 
DO NOT output "thought:" or any other prefix.

<think>
1. 我是谁? 读取人设。
2. 上文发生了什么?
3. 现在的时间是? (当前时间: ${new Date().toLocaleString()})
4. 我要回复的人是? (我对她的称呼应该是什么? 不能是什么?)
5. 对于user发出的消息如果是char(人物)，应该会怎样回复? 这样回复是否符合char的人设，是否符合user的喜好，是否触及雷点禁区，是否会OOC? 确认每一个问题回复均合理则进入下一步，否则再次思考回复内容。
6. 思考回复格式。用什么语言。是否为线上模式，如果是，分句子发出。是否为线下模式(带动作环境描写)，如果是线下，按线下格式发出。
7. 最后确认。发出的消息是否贴合时间，是否贴合char的人设，是否符合user喜好，格式是否正确。
8. 确认无误，发送。
</think>

After your <think> process, output your final response.
Follow these rules strictly for your final response:
1. DO NOT output a single large block of text. Break your response into short, natural sentences or phrases, as if sending multiple separate messages.
2. Output each separate message on a new line.
3. Keep it casual, conversational, and in character.
4. Do not include prefixes like "AI:" or "${chat.trueName || chat.name}:" in your output, just the message content.
`;
    ...
    try {
      ...
      // Fallback: if the model still leaked "thought, ..." or "thought: ...", remove it up to the first blank line.
      if (/^(?:thought|thinking|【thought】)/i.test(text)) {
        const parts = text.split('\n\n');
        if (parts.length > 1) {
          text = parts.slice(1).join('\n\n').trim();
        } else {
          // If no blank line, just clear it to be safe, or leave it if we can't parse it.
          // Usually it's better to just return the text and let the user see the error than swallow the whole message.
        }
      }
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // Delay based on length, min 1 second
        const delay = Math.max(1000, line.length * 100);
        await new Promise(resolve => setTimeout(resolve, delay));
        
        let newMsg: Message;
        const stickerMatch = line.match(/^\[表情包[:：]\s*(.+?)\s*\]$/);
        if (stickerMatch) {
          const stickerName = stickerMatch[1].trim();
          const sticker = boundStickers.find(s => s.name.trim() === stickerName);
          if (sticker) {
            newMsg = {
        ...
        if (!isMounted.current) {
          try {
            const currentMessagesStr = window.localStorage.getItem(`sjzn_msgs_${chat.id}`);
            const currentMessages = currentMessagesStr ? JSON.parse(currentMessagesStr) : [];
            window.localStorage.setItem(`sjzn_msgs_${chat.id}`, JSON.stringify([...currentMessages, newMsg]));
          } catch (e) {
            console.error(e);
          }
          onBackgroundReply(chat.id, newMsg);
          window.dispatchEvent(new CustomEvent('sjzn_bg_message', { detail: { chatId: chat.id, message: newMsg } }));
        } else {
      ...
      alert(`AI 回复生成失败: ${error.message}\n请检查 API 地址、密钥或模型是否支持识图（如果开启了识图）。`);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    ...
    >
      {/* Header */}
      <header className="pt-6 pb-1.5 px-4 bg-[#F3F3F3]/90 backdrop-blur-sm border-b border-slate-200 flex items-center justify-between relative">
        <button onClick={onBack} className="flex items-center text-slate-800 active:opacity-50 transition-opacity relative z-10">
          <ChevronLeft size={28} />
        </button>
        <h2 className="text-[17px] font-semibold text-slate-800 absolute left-1/2 -translate-x-1/2">
           ...
           messages.map((msg, index) => {
             ...
             return (
               <React.Fragment key={msg.id}>
                 ...
                 <div id={`msg-${msg.id}`} className={`flex gap-3 mb-4 ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                   {msg.sender === 'ai' ? (
                     chat.avatar ? (
                       <img src={chat.avatar} className="w-10 h-10 object-cover border border-slate-100 shadow-sm bg-slate-100 flex-shrink-0" style={{ borderRadius: `${chat.avatarRadius === undefined ? 24 : chat.avatarRadius / 2}%` }} />
                     ) : (
                ...
                {stickerGroups.map(g => (
                  <div key={g.id} className="relative flex items-center">
                    <button 
                      ...
                      className={`px-3 py-1.5 rounded-full text-sm whitespace-nowrap transition-colors ${activeGroupId === g.id ? 'bg-white text-slate-800 shadow-sm font-medium border border-slate-200' : 'text-slate-500 hover:bg-slate-200/50 border border-transparent'} ${isManagingStickers && g.id !== 'default' ? 'pr-8' : ''}`}
                    >
                      {g.name}
                    </button>
                    {isManagingStickers && g.id !== 'default' && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setGroupToDelete(g);
                        }}
                        className="absolute right-1 w-6 h-6 rounded-full bg-red-100 text-red-500 flex items-center justify-center hover:bg-red-200 transition-colors"
                      >
            ...
            <div className="flex-1 overflow-y-auto p-3">
              <div className="grid grid-cols-5 gap-3">
                ...
                {activeStickers.map(sticker => (
                  ...
                  >
                    <div className={`aspect-square w-full rounded-xl overflow-hidden bg-white shadow-sm border ${isManagingStickers && selectedStickers.includes(sticker.id) ? 'border-slate-800 ring-2 ring-slate-800/20' : 'border-slate-200'}`}>
                      <img src={sticker.url} alt={sticker.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    </div>
                    <span className="text-[10px] text-slate-500 truncate w-full text-center">{sticker.name}</span>
                    
                    {isManagingStickers && (
                      <div className={`absolute top-1 right-1 w-4 h-4 rounded-full border flex items-center justify-center ${selectedStickers.includes(sticker.id) ? 'bg-slate-500 border-slate-800' : 'bg-white/80 border-slate-300'}`}>
              ...
              <div className="px-3 py-2 border-t border-slate-200 flex items-center justify-between bg-white">
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="text-sm text-slate-800 px-3 py-1.5 disabled:opacity-50 font-medium"
                  >
      ...
      <AnimatePresence>
        {groupToDelete && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[80] bg-black/40 flex items-center justify-center p-10"
          ...
          >
            <motion.div 
              ...
              exit={{ scale: 0.95, opacity: 0 }}
              onClick={e => e.stopPropagation()}
              className="bg-white/90 backdrop-blur-xl rounded-2xl w-full max-w-[270px] overflow-hidden shadow-2xl flex flex-col items-center pt-5"
            >
              <h3 className="text-[17px] font-semibold text-black mb-1">删除分组</h3>
              <p className="text-[13px] text-black/70 text-center px-4 mb-5">
                确定要删除“{groupToDelete.name}”吗？<br/>该分组下的所有表情包也将被删除。
              </p>
              <div className="w-full flex border-t border-black/10">
                <button 
                  onClick={() => setGroupToDelete(null)}
                  className="flex-1 py-3 text-[17px] text-slate-600 border-r border-black/10 active:bg-black/5 transition-colors"
                >
                  取消
                </button>
                <button 
                  onClick={() => {
                    setStickers(stickers.filter(s => s.groupId !== groupToDelete.id));
                    setStickerGroups(stickerGroups.filter(g => g.id !== groupToDelete.id));
                    if (activeGroupId === groupToDelete.id) {
                      setActiveGroupId('default');
                    }
                    setGroupToDelete(null);
                  }}
                  className="flex-1 py-3 text-[17px] text-red-500 font-semibold active:bg-black/5 transition-colors"
                >
                  删除
                </button>
              </div>
            ...
            >
              ...
              <div className="p-6">
                <div className="flex justify-between items-center">
                  ...
                  >
                    {recognizeUrl ? <ToggleRight size={32} className="text-slate-800" /> : <ToggleLeft size={32} className="text-slate-400" />}
                  </button>
                </div>
              </div>
            </motion.div>
              ...
              <div className="p-2 max-h-60 overflow-y-auto">
                {stickerGroups.map(g => (
                  <button 
                    ...
                    className={`w-full text-left px-4 py-3 rounded-xl text-[15px] flex items-center justify-between ${targetMoveGroupId === g.id ? 'bg-slate-100 text-slate-800 font-medium' : 'text-slate-700 hover:bg-slate-50'}`}
                  >
                    {g.name}
...
export default function App() {
  ...
  const [notification, setNotification] = useState<{chat: ChatItem, message: string} | null>(null);
  ...
  const handleBackgroundReply = (chatId: string, message: Message) => {
    setChats(prevChats => {
      ...
      const updatedChat = {
        ...chat,
        lastMsg: message.type === 'text' ? message.content : '[图片]',
        time: new Date(message.timestamp).toISOString(),
        timestamp: message.timestamp,
        unread: activeChatRef.current?.id === chatId ? 0 : (chat.unread || 0) + 1
      };
      ...
      if (!updatedChat.isPinned) {
        const pinnedChats = newChats.filter(c => c.isPinned);
        const unpinnedChats = newChats.filter(c => !c.isPinned && c.id !== chatId);
    ...
    setChats(prevChats => {
      const chatForNotif = prevChats.find(c => c.id === chatId);
      if (chatForNotif && activeChatRef.current?.id !== chatId) {
        setNotification({ chat: chatForNotif, message: message.type === 'text' ? message.content : '[图片]' });
      }
      return prevChats;
    });
  };
  ...
  const handleUpdateChat = (id: string, updates: Partial<ChatItem>) => {
    setChats(prevChats => prevChats.map(chat => 
      chat.id === id ? { ...chat, ...updates } : chat
    ));
    setActiveChat(prev => prev && prev.id === id ? { ...prev, ...updates } : prev);
  };

  const renderContent = () => {
    switch (activeTab) {
      case 'chat': return <ChatView chats={chats} onPin={handlePinChat} onDelete={handleDeleteChat} onChatClick={handleChatClick} />;
      case 'mini': return <MiniProgramView />;
      case 'discover': return <DiscoverView />;
      case 'me': return <ProfileView onOpenSettings={() => setShowSettings(true)} onOpenWorldBook={() => setShowWorldBook(true)} />;
    }
  };
  ...
  return (
    <div className="flex justify-center bg-slate-100 min-h-screen">
      ...
      <div className="relative w-full max-w-[430px] bg-milk-yellow/30 min-h-screen flex flex-col shadow-2xl overflow-hidden">
        ...
        <AnimatePresence>
          {notification && (
            <motion.div
              ...
              className="absolute top-4 left-4 right-4 bg-white rounded-2xl shadow-lg p-4 z-[100] flex items-center gap-3 cursor-pointer"
              onClick={() => {
                handleChatClick(notification.chat);
                setNotification(null);
              }}
            >
              {notification.chat.avatar ? (
                <img src={notification.chat.avatar} className="w-12 h-12 object-cover border border-slate-100" style={{ borderRadius: `${notification.chat.avatarRadius === undefined ? 24 : notification.chat.avatarRadius / 2}%` }} />
              ) : (
                <div className="w-12 h-12 bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400" style={{ borderRadius: `${notification.chat.avatarRadius === undefined ? 24 : notification.chat.avatarRadius / 2}%` }}>
                  <User size={24} />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <h4 className="font-semibold text-slate-800">{notification.chat.remark || notification.chat.name}</h4>
        ...
        {/* World Book Overlay */}
        <AnimatePresence>
          {showWorldBook && <WorldBookView onBack={() => setShowWorldBook(false)} />}
        </AnimatePresence>
