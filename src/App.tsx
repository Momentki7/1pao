...
const ApiConfigBlock = ({ 
  ...
  const datalistId = `models-${title.replace(/\s+/g, '-')}`;
      ...
      <div className="mb-3">
        ...
        <div className="flex gap-2">
          <input 
            ...
            value={config.model}
            onChange={(e) => onChange({ ...config, model: e.target.value, presetId: '' })}
          />
...
const MinimaxConfigBlock = ({ 
  ...
  const [fetchedModels, setFetchedModels] = useState<string[]>([]);
  const [isFetching, setIsFetching] = useState(false);
  const datalistId = "models-minimax";
...
const ChatRoom = ({ chat, onBack }: { chat: ChatItem, onBack: () => void }) => {
  ...
  const handleRequestReply = async () => {
    ...
    const contextStr = messages.map(m => `${m.sender === 'user' ? 'User' : chat.name}: ${m.type === 'text' ? m.content : '[Sticker]'}`).join('\n');
